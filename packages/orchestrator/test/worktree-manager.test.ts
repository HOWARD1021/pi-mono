...
describe("WorktreeManager", () => {
	it("create() runs git worktree add and captures base SHA", () => {
		...
		expect(mockExec).toHaveBeenCalledWith(expect.stringContaining("git rev-parse"), expect.any(Object));
		expect(mockExec).toHaveBeenCalledWith(expect.stringContaining("git worktree add"), expect.any(Object));
	});
	...
	it("reset() runs git reset --hard to stored SHA", () => {
		...
		expect(mockExec).toHaveBeenCalledWith(expect.stringContaining("git reset --hard abc1234"), expect.any(Object));
		expect(mockExec).toHaveBeenCalledWith(expect.stringContaining("git push --force-with-lease"), expect.any(Object));
	});
	...
	it("remove() runs git worktree remove", () => {
		...
		expect(mockExec).toHaveBeenCalledWith(expect.stringContaining("git worktree remove"), expect.any(Object));
	});
});
