...
| `app-factory/` | OpenSpec-style app workflow with intake, spec artifacts, approval, and task-by-task execution commands |
...
| `custom-provider-local-cli/` | Spawn a local CLI process as a text-only model provider (defaults to Claude CLI style args) |
